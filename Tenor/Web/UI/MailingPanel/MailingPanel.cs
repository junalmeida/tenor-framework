		...
		namespace UI
		{
			namespace WebControls
			{
				namespace Core
				{
					...
					/// <summary>
					/// Controle com funções para enviar e-mails.
					/// </summary>
					/// <remarks></remarks>
					[ToolboxData("<{0}:MailingPanel runat=server></{0}:MailingPanel>")]public class MailingPanel : MultiView
					{
						
						
						
						[DefaultValue(true)][Description("Indica se deve usar o Text das checkboxes como identificador de campo."), Category("Behavior")]public bool UseTextForCheckbox
						{
							get
							{
						...
						/// <returns></returns>
						/// <remarks></remarks>
						[DefaultValue("Não foi possível enviar a mensagem. Por favor tente novamente mais tarde.")][Description("Mensagem de erro a ser exibida quando o processo não tiver sucesso."), Category("Behavior")]public string ErrorMessage
						{
							get
							{
						...
						/// <returns></returns>
						/// <remarks></remarks>
						[Description("Prefixo dos campos que serão utilizados no envio da mensagem. Caso esta propriedade não seja preenchida, todos os campos serão usados."), Category("Behavior")]public string FieldPrefix
						{
							get
							{
						...
						/// <returns></returns>
						/// <remarks></remarks>
						[TypeConverter(typeof(AssociatedControlConverter)), IDReferenceProperty()][Description("Nome de um IButtonControl para ativar a ação de envio."), Category("Behavior")]public string SendButton
						{
							get
							{
						...
						/// <returns></returns>
						/// <remarks></remarks>
						[Browsable(false)]public FormView FormView
						{
							get
							{
								FormView vw = null;
						...
						/// <returns></returns>
						/// <remarks></remarks>
						[Browsable(false)]public ResultView ResultView
						{
							get
							{
								ResultView vw = null;
						...
						protected void SendMessage()
						{
							try
							{
								...
								foreach (string key in msg.TemplateValues.Keys)
								{
									...
									if (msg.TemplateValues[key].Length > Tenor.Configuration.MailMessage.MaxCharTemplateValue)
									{
										mensagem += "- O campo " + key + " só pode ter até " + Tenor.Configuration.MailMessage.MaxCharTemplateValue.ToString() + " caracteres." + Environment.NewLine;
									}
								}
