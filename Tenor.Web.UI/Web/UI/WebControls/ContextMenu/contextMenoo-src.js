...
contextMenoo = new Class({
	options:{
		selector: '.contextmenu', className: '.protoMenu', pageOffset: 25, fade: false
	},
	initialize: function (op) {
		...
		this.options.menuItems.each(function(item){
			this.cont.adopt(item.separator ? 
				...
				new Element('a', {	'href': '#', 'title': item.name, 'class': item.disabled ? 'disabled' : ''}).addEvent('click', this.onClick.bindWithEvent(this,[item.callback])).setHTML(item.name))
		}.bind(this));
		this.Fade.set(0);
